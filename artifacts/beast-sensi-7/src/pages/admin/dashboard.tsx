...
import { GenerateLicenseInputDuration } from "@workspace/api-client-react/src/generated/api.schemas";

function PhotoUploadPicker({
  value,
  onChange,
  token,
  index,
  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <button
        ...
        disabled={uploading}
        className="relative flex-shrink-0 w-12 h-12 rounded-lg border-2 border-dashed border-primary/40 bg-black/40 hover:border-primary hover:bg-primary/10 transition-all overflow-hidden flex items-center justify-center"
        title={`Subir foto ${index + 1}`}
      >
        {uploading ? (
      ...
      <Input
        ...
        className="bg-black/60 border-primary/30 text-white focus:border-primary focus-visible:ring-primary font-mono text-xs flex-1"
      />
    </div>
  );
}
...
export function AdminDashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-background pb-12 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 mt-8 space-y-8 relative z-10">
        {/* Stats Row */}
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          <StatCard title="Alertas" value={suspiciousData?.attempts?.length ?? 0} icon={<span className="text-red-400">🚨</span>} loading={false} subtitle="intentos sospechosos" />
          <StatCard title="Sensi Generadas" value={sensiCount?.count} icon={<Zap className="text-yellow-400" />} loading={false} />
          <StatCard title="Modif Generadas" value={modifCount?.count} icon={<Wrench className="text-purple-400" />} loading={false} />
          <StatCard title="Valoraciones" value={ratingsSummary?.total} icon={<Star className="text-yellow-400" />} loading={false} subtitle={ratingsSummary?.average ? `★ ${ratingsSummary.average}/5` : undefined} />
        </div>
                  ...
                  <div key={a.id} className="rounded-lg bg-black/60 border border-red-500/20 p-3 flex flex-col sm:flex-row sm:items-center gap-2 text-xs">
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="text-zinc-400">
                        <span className="text-green-400">Dueño:</span> {a.originalUsername || "desconocido"} — {a.originalDevice?.substring(0, 20) ?? "—"}
                      </div>
                      <div className="text-zinc-400">
                        <span className="text-red-400">Intento:</span> {a.attemptUsername || "sin nombre"} — {a.attemptDevice?.substring(0, 20) ?? "—"}
                      </div>
                    </div>
              ...
              <form onSubmit={handleCreateDownload} className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-xs uppercase text-muted-foreground tracking-widest font-medium flex items-center gap-1">
                    ...
                    <span className="text-purple-400 font-bold">Imagen</span> — URL de portada (opcional)
                  </label>
                  <Input
                    value={dlImage}
                    onChange={e => setDlImage(e.target.value)}
                    placeholder="https://i.imgur.com/... o link directo a imagen"
                    className="bg-black/60 border-primary/30 text-white focus:border-primary focus-visible:ring-primary font-mono text-xs"
                  />
                  {dlImage && (
                    <div className="mt-1 rounded-lg overflow-hidden border border-primary/30 bg-black/40">
                      <img src={dlImage} alt="preview" className="w-full h-32 object-cover" onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                    </div>
                  )}
                </div>
                <div className="space-y-2">
        ...
        <Card className="bg-[rgba(255,255,255,0.02)] backdrop-blur-md border border-primary/20 shadow-[0_0_20px_rgba(139,92,246,0.05)]">
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-muted-foreground">Pega la URL de la imagen (Imgur, Drive, etc.) que quieres usar como banner de cada catálogo. Si lo dejas vacío se usa la imagen por defecto.</p>
            <div className="grid sm:grid-cols-2 gap-3">
              {[
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <Card className="bg-[rgba(255,255,255,0.02)] backdrop-blur-md border border-primary/20 shadow-[0_0_20px_rgba(139,92,246,0.05)]">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3 p-4 rounded-xl border border-primary/10 bg-black/30">
                ...
                />
                <Input
                  placeholder="URL del QR (opcional)"
                  value={hudForm.qrUrl}
          ...
          <Card className="bg-[rgba(255,255,255,0.02)] backdrop-blur-md border border-primary/20 shadow-[0_0_20px_rgba(139,92,246,0.05)]">
            ...
            <CardContent className="space-y-4">
              <p className="text-xs text-zinc-500 font-mono">Selecciona el modelo y carga sus 11 fotos. Cada modelo tiene sus propias fotos en la sección VIP.</p>

              {/* Selector de modelo */}
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-widest text-zinc-400 font-bold">Modelo de iPhone</label>
                <select
                  value={modSelectedModel}
                  onChange={e => setModSelectedModel(e.target.value)}
              ...
              {/* 11 inputs de foto para el modelo seleccionado */}
              <div className="space-y-2 p-4 rounded-xl border border-primary/10 bg-black/30">
                <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 mb-3">
                  Fotos para <span className="text-primary font-bold">{modSelectedModel}</span>
                </p>
                {Array.from({ length: 11 }, (_, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      value={(modPhotos[modSelectedModel] ?? [])[i] ?? ""}
                      onChange={e => {
                        const current = [...(modPhotos[modSelectedModel] ?? Array(11).fill(""))];
                        current[i] = e.target.value;
                        setModPhotos(prev => ({ ...prev, [modSelectedModel]: current }));
                      }}
                      className="bg-black/60 border-primary/30 text-white focus:border-primary focus-visible:ring-primary font-mono text-xs"
                    />
                  </div>
              ...
              <Button
                onClick={() => {
                  saveSettingMutation.mutate(
                    { key: "modificacion_photos", value: JSON.stringify(modPhotos) },
                    {
                      onSuccess: () => toast({ title: "Fotos guardadas", description: `Fotos de ${modSelectedModel} actualizadas.` }),
                      onError: () => toast({ title: "Error", description: "No se pudo guardar.", variant: "destructive" }),
                    }
                  );
                }}
              ...
              >
                {saveSettingMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Save className="w-4 h-4 mr-2" />}
                GUARDAR FOTOS DE {modSelectedModel.toUpperCase()}
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="bg-[rgba(255,255,255,0.02)] backdrop-blur-md border border-primary/20 shadow-[0_0_20px_rgba(139,92,246,0.05)]">
          ...
          <CardContent className="space-y-5">
            <p className="text-xs text-muted-foreground">Añade testimonios reales que aparecen en la página de Referencias. Pon la URL de la foto del usuario (puede ser link de WhatsApp, Imgur, etc).</p>
            ...
            <div className="space-y-3 p-4 rounded-xl border border-primary/10 bg-black/30">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const newRefs = [...referencias, { ...refForm }];
                  ...
                  saveSettingMutation.mutate(
                    ...
                    {
                      onSuccess: () => toast({ title: "Guardado", description: "Testimonio agregado a Referencias." }),
                      onError: () => toast({ title: "Error", description: "No se pudo guardar.", variant: "destructive" }),
                    }
                  );
                }}
